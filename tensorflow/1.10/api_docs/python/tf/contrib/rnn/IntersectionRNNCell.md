...
* <b>`new_y`</b>: batch x num units, Tensor representing the output of the +RNN
    after reading `inputs` when previous state was `state`.
* <b>`new_state`</b>: batch x num units, Tensor representing the state of the +RNN
    after reading `inputs` when previous state was `state`.
...
* <b>`ValueError`</b>: If input size != output size (these must be equal when
