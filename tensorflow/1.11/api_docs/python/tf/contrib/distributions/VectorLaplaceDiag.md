...
<meta itemprop="name" content="tf.contrib.distributions.VectorLaplaceDiag" />
...
Defined in [`tensorflow/contrib/distributions/python/ops/vector_laplace_diag.py`](https://www.tensorflow.org/code/tensorflow/contrib/distributions/python/ops/vector_laplace_diag.py).
...
Construct Vector Laplace distribution on `R^k`. (deprecated)
