...
API links are converted when the site is published. To link to the python API simply enclose the full symbol path in backticks: \`tf.data.Dataset\` to produce <a href="../api_docs/python/tf/data/Dataset.md"><code>tf.data.Dataset</code></a>. For the C++ API use the namespace path: \`tensorflow::Tensor\`  to produce `tensorflow::Tensor`.
...
`api_docs/python/tf/image/decode_png.md`:
    ...
    * **`dtype`**: An optional <a href="../api_docs/python/tf/DType.md"><code>tf.DType</code></a> from: `tf.uint8,
